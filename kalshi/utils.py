...
def plot_trades(
    ...
    for _, trade in df.iterrows():
        ...
        if trade["taker_side"] == "yes":
            ...
            ax.annotate(
                "",
                xy=(arrow_x, y + arrow_size),
                xytext=(arrow_x, y - arrow_size),
                arrowprops={
                    "arrowstyle": "->",
                    "color": "green",
                    "lw": 2,
            ...
            ax.annotate(
                ...
                xy=(arrow_x, y - arrow_size),
                xytext=(arrow_x, y + arrow_size),
                arrowprops={"arrowstyle": "->", "color": "red", "lw": 2, "alpha": 0.9},
            )
...
def calculate_vwap(
    trades_df: pd.DataFrame, price_col: str = "yes_price", volume_col: str = "count"
) -> float:
    """
...
def calculate_volume_stats(trades_df: pd.DataFrame) -> dict[str, float]:
    ...
    yes_taker_dollar_volume = (
        trades_df[trades_df["taker_side"] == "yes"]["yes_price"]
        * trades_df[trades_df["taker_side"] == "yes"]["count"]
    ).sum() / 100
    # For no takers, they pay the no_price which is (100 - yes_price)
    no_taker_dollar_volume = (
        (100 - trades_df[trades_df["taker_side"] == "no"]["yes_price"])
        * trades_df[trades_df["taker_side"] == "no"]["count"]
    ).sum() / 100
    ...
    return {
        ...
        "yes_taker_percentage": yes_taker_volume / total_volume * 100
        if total_volume > 0
        else 0,
        "no_taker_percentage": no_taker_volume / total_volume * 100
        if total_volume > 0
        else 0,
        ...
        "yes_dollar_percentage": yes_taker_dollar_volume / total_dollar_volume * 100
        if total_dollar_volume > 0
        else 0,
        "no_dollar_percentage": no_taker_dollar_volume / total_dollar_volume * 100
        if total_dollar_volume > 0
        else 0,
    }
